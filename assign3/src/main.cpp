...
double lerp(double a0, double a1, double w)
{
    ...
    double linear = (1.0 - w) * a0 + w * a1;
    double cubic = (a1 - a0) * (3.0 - w * 2.0) * w * w + a0;
    return cubic;
}
...
double dotGridGradient(int ix, int iy, double x, double y)
{
  ...
  GradientVector gradients[grid_size][grid_size];
  GradientVector& gradient = gradients[ix][iy];
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    const Vector3d pgram_normal = pgram_u.cross(pgram_v);
    const double denom = ray_direction.dot(pgram_normal);
    ...
    const double t = (pgram_origin - ray_origin).dot(pgram_normal) / denom;

    const Vector3d intersection_point = ray_origin + t * ray_direction;
    const Vector3d p_vec = intersection_point - pgram_origin;

    const double u = p_vec.dot(pgram_u) / pgram_u.dot(pgram_u);
    const double v = p_vec.dot(pgram_v) / pgram_v.dot(pgram_v);
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    if (closest_index != -1)
    {
        ...
        double distance_to_intersection = (p - ray_origin).norm();
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
      ...
      if (is_light_visible(p, Li, light_position))
      {
        ...
        //const Vector3d H = (Li + (ray_origin - p).normalized()).normalized();
        const Vector3d R = (2.0 * N.dot(Li) * N - Li).normalized();
        const double specular_intensity = std::pow(std::max(R.dot((ray_origin - p).normalized()), 0.0), obj_specular_exponent);
        const Vector4d specular = obj_specular_color * specular_intensity;
    ...
    if (max_bounce > 0)
    {
      const Vector3d reflection_direction = (2.0 * N.dot(ray_direction) * N - ray_direction).normalized();

    //shoot reflected ray recursively
      const Vector4d reflected_color = shoot_ray(p, reflection_direction, max_bounce - 1);

      reflection_color = reflected_color.cwiseProduct(refl_color);
    }
...
void raytrace_scene()
{
    ...
    double image_y = image_z;
    double image_x = image_z * aspect_ratio;
    ...
    for (unsigned i = 0; i < w; ++i)
    {
        for (unsigned j = 0; j < h; ++j)
        {
            ...
            //commented my attempt on depth of field
            //double random_x = (double)rand() / RAND_MAX - 0.5;
            //double random_y = (double)rand() / RAND_MAX - 0.5;
            //Vector3d aperture_displacement = camera_aperture * (random_x * x_displacement + random_y * y_displacement);

            const Vector3d pixel_center = image_origin + (i + 0.5) * x_displacement + (j + 0.5) * y_displacement;
            //ray_origin = camera_position + aperture_displacement;
